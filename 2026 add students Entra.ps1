...
function New-OrUpdateStudent {
    ...
    $existing = Get-ADUser -Filter "SamAccountName -eq '$Username'" -Properties DistinguishedName, Enabled, UserAccountControl, GivenName, Surname, DisplayName, Department, Description, UserPrincipalName, EmployeeID, EmployeeType, pwdLastSet -ErrorAction SilentlyContinue
    ...
    try {
        if ($null -ne $existing) {
            ...
            if ($existing.GivenName -ne $FirstName -or
                ...
                Set-ADUser $existing -GivenName $FirstName -Surname $LastName `
                    -DisplayName $FullName -Department "Year $YearLevel" `
                    -Description "2026 Year $YearLevel" `
                    -UserPrincipalName $RequiredUserPrincipalName -EmailAddress $RequiredUserPrincipalName -ErrorAction Stop
            ...
            if ($existing.DistinguishedName -notlike "*$OU*") {
                ...
                Move-ADObject $existing.DistinguishedName -TargetPath $OU -ErrorAction Stop
            ...
            if ($existing.employeeType -ne "Student" -or [string]::IsNullOrWhiteSpace($existing.employeeID)) {
                ...
                Set-ADUser $Username -Replace @{employeeType = "Student"; employeeID = $EmployeeID} -ErrorAction Stop
            ...
            if (-not $existing.pwdLastSet -or $existing.pwdLastSet -eq 0) {
                ...
                $Password = Invoke-RestMethod https://www.dinopass.com/password/simple
                Set-ADAccountPassword -Identity $Username -NewPassword (ConvertTo-SecureString $Password -AsPlainText -Force) -Reset -ErrorAction Stop
                Set-ADUser $Username -ChangePasswordAtLogon $true -ErrorAction Stop
            ...
            Add-ADGroupMember "Students" $Username -ErrorAction Stop
            Update-Status "Step 10: Adding $Username to Year $YearLevel Students group"
            Add-ADGroupMember "Year $YearLevel Students" $Username -ErrorAction Stop

            $Result = "Updated"
        }
        else {
            Update-Status "Step 2: Account does not exist in AD: $Username"
            ...
            New-ADUser -Name $FullName -GivenName $FirstName -Surname $LastName `
                -SamAccountName $Username -UserPrincipalName $Email -DisplayName $FullName `
                -Path $OU -AccountPassword (ConvertTo-SecureString $Password -AsPlainText -Force) `
                -Enabled $true -Department "Year $YearLevel" -Description "2026 Year $YearLevel" `
                -Company "Hope Christian College" -ErrorAction Stop
...
$CreateButton.Add_Click({
        ...
        for ($i=0; $i -lt 5; $i++) {
                ...
                if ($name -and $year -ne $null) { $entries += [pscustomobject]@{Name=$name; Year=$year} }
        }
        ...
        Update-Status "Done: $created created, $updated updated, $failed failed`nSee $ErrorLogPath for details." 100
        if ($failed -gt 0) { [System.Windows.MessageBox]::Show("Processing complete. $created created, $updated updated, $failed failed.`nSee $ErrorLogPath for details.", 'Summary') } else { [System.Windows.MessageBox]::Show("Processing complete. $created created, $updated updated, $failed failed.", 'Summary') }
})
