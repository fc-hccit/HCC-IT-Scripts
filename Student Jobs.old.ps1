...
Send-MailMessage -From "IT Support <itsupport@hopecc.sa.edu.au>" -To "$stafffirst.$stafflast@hopecc.sa.edu.au" -Subject "$studentfirst $studentlast IT Update" -Body "Hi $stafffirst, `n`nThis is an update regarding $studentfirst $studentlast's IT issue.`n`n$updatetext. `n`n$loandevicegiven." -SmtpServer "aspmx.l.google.com"

}
Function completedmail {

Send-MailMessage -From "IT Support <itsupport@hopecc.sa.edu.au>" -To "$stafffirst.$stafflast@hopecc.sa.edu.au" -Subject "$studentfirst $studentlast IT Update" -Body "Hi $stafffirst, `n`n$studentfirst $studentlast's IT issue has been resolved.`n`n$updatetext." -SmtpServer "aspmx.l.google.com"
...
Rename-Item -Path $jobspath\$logname -newname "#$loandevice_$logname"
}
...
Write-Output "$date $updatetext" | Out-File $jobspath\$logname -Append 
Rename-Item -path $jobspath\$logname -newname "#$logname"
}
