...
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$emailrecipientname <$emailrecipient@$Domain>" -Subject "$username's battery level under 80%" -Body "Hi Jarrod, `n`nJust letting you know that $username's laptop had a battery level of $percentage% when they first logged in at $Time." -SmtpServer "aspmx.l.google.com"

Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$name <$username.student@$Domain>" -Subject "It looks like you didn't charge your battery last night!" -Body "Hi $firstname, `n`nWe noticed you just logged in and your laptop's battery level was $percentage%`n`nYou must remember to fully charge your laptop every night so its ready for the next day.`n`nYou will receive a consequense if you keep forgetting to charge your laptop." -SmtpServer "aspmx.l.google.com"
