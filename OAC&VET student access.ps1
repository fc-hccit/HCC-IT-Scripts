...
$usergroups = Get-ADPrincipalGroupMembership $username | select name | Where-Object {$_.name -like '*gpOpenAccess&VET*' }

$Usergroupname = $usergroups.name

if($Usergroupname -like 'gpOpenAccess&VET') {
...
if($Usergroupname -like 'gpOpenAccess&VET') {Remove-ADGroupMember -Identity 'gpOpenAccess&VET' -Members $username -Confirm:$False}

else {Add-ADGroupMember -Identity 'gpOpenAccess&VET' -Members $username -Confirm:$False}
...
$users = (Get-ADUser -Filter {Enabled -eq $TRUE} -SearchBase "OU=Senior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au").SamAccountName
...
$Form.text                       = "OAC & VET Student Access Status"
$Form.TopMost                    = $true
...
$Label1.Font                     = New-Object System.Drawing.Font('Microsoft Sans Serif',18,[System.Drawing.FontStyle]([System.Drawing.FontStyle]::Bold))
