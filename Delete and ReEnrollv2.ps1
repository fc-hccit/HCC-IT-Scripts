...
Write-Host "`nSearching for and removing device..." -ForegroundColor Cyan
Write-Host "`nSign into your Microsoft Account" -ForegroundColor Cyan

# Remove from Autopilot
$ap = Get-MgBetaDeviceManagementWindowsAutopilotDeviceIdentity -Filter "contains(serialNumber,'$serial')" | Where-Object { $_.SerialNumber -eq $serial }
if ($ap) {
    Remove-MgBetaDeviceManagementWindowsAutopilotDeviceIdentity -WindowsAutopilotDeviceIdentityId $ap.Id
...
# Remove from Intune
$md = Get-MgDeviceManagementManagedDevice -Filter "contains(serialNumber,'$serial')" | Where-Object { $_.SerialNumber -eq $serial }
if ($md) {
...
do {
    ...
    $stillExists = (
        (Get-MgBetaDeviceManagementWindowsAutopilotDeviceIdentity -Filter "contains(serialNumber,'$serial')" | Where-Object { $_.SerialNumber -eq $serial }) -or
        (Get-MgDeviceManagementManagedDevice -Filter "contains(serialNumber,'$serial')" | Where-Object { $_.SerialNumber -eq $serial })
    )
...
do {
    ...
    $device = Get-MgBetaDeviceManagementWindowsAutopilotDeviceIdentity -Filter "contains(serialNumber,'$serial')" | Where-Object { $_.SerialNumber -eq $serial }
    ...
    switch ($status) {
        ...
        "assignedUnknownSyncState" { Write-Host "Status: assigned" -ForegroundColor Green }
    }

} until (
    $status -eq "assignedUnknownSyncState" -or
    $attempt -ge $maxAttempts
)

if ($status -eq "assignedUnknownSyncState") {
