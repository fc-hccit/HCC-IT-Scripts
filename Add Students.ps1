...
$EmailrecipientIT = "itstaff"
$EmailrecipientITname = "IT Staff"
$EmailrecipientJP = "jpteachingstaff"
$EmailrecipientJPname = "Junior Primary Teaching Staff"
$EmailrecipientPS = "primaryteachingstaff"
$EmailrecipientPSname = "Primary School Teaching Staff"
$Wait = Start-Sleep 3
$logpath = "$env:HOMEPATH\Desktop\adduser.log"
$pwdpath = "$env:HOMEPATH\Desktop\New User Passwords $date.csv"
$Workstations = "HCC-DC01,HCC-DC02,AVZ4860-MUS01,AVZ4860-MUS02,AVZ4860-MUS03,AVZ4860-MUS04,AVZ4860-MUS05,AVZ4860-MUS06,AVZ4860-MUS07,AVZ4860-MUS08,AVZ4860-MUS09,AVZ4860-MUS10,AVZ4860-MUS11,AVZ4860-MUS12,AVZ4860-MUS13,AVZ4860-MUS14,AVZ4860-MUS15"

Start-Transcript -path $logpath -append

$Status.text = "Counting lines in file"
...
if(!($fn -in $headers)){[System.Windows.Forms.MessageBox]::Show("No Firstname Header Found!", 'Error', 'OK', 'Error');$Status.text = "Ready..."}

elseif(!($ln -in $headers)){[System.Windows.Forms.MessageBox]::Show("No Lastname Header Found!", 'Error', 'OK', 'Error');$Status.text = "Ready..."}

elseif(!($yr -in $headers)){[System.Windows.Forms.MessageBox]::Show("No Year Header Found!", 'Error', 'OK', 'Error');$Status.text = "Ready..."}
...
$confirm = [System.Windows.Forms.MessageBox]::Show("You are about to add $LinesInFile students from the following CSV!`n $csvfile`n", 'Please Confirm', 'YesNo', 'Warning')
...
$CSVObject | Export-CSV -path $pwdpath -Append -NoTypeInformation
...
$Wait
New-ADUser -Name "$SF $SL"  -UserPrincipalName "$SF.$SL@$Domain" -SamAccountName "$SF.$SL" -GivenName $SF -DisplayName "$SF $SL" -SurName "$SL" -Description "Junior School" -EmailAddress "$SF.$SL.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$SF.$SL" -Office "$SY" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $SY,OU=Junior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
...
$Wait
gam create user "$SF.$SL.student@$Domain" firstname "$SF" lastname "$SL" password "$simplepass" changepassword "on" org "Primary School (No Mail)/Year $SY Students" organization department "Year 0$SY" primary
...
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientJPname <$EmailrecipientJP@$Domain>" -Subject "A login has been created for $SF $SL" -Body "Hi $EmailrecipientJPname, `n`nI've just created a computer and email account for $SF $SL who will be in year $SY.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}

elseif ($SY -In 3..5) { 
$Status.text = "Creating AD Account for $SF $SL.."
$Wait
New-ADUser -Name "$SF $SL"  -UserPrincipalName "$SF.$SL@$Domain" -SamAccountName "$SF.$SL" -GivenName $SF -DisplayName "$SF $SL" -SurName "$SL" -Description "Primary School" -EmailAddress "$SF.$SL.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$SF.$SL" -Office "$SY" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $SY,OU=Primary School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
$Status.text = "Adding User $SF $SL to Year $SY Students Group.."
$Wait
Add-ADGroupMember -Identity "Year $SY Students" -Members "$SF.$SL" -PassThru
$Status.text = "Adding User $SF $SL to gpOUStudentsYr$SY Group.."
...
$Status.text = "Set Allowed Workstations"
$Wait
Set-ADUser -Identity "$SF.$SL" -LogonWorkstations "$Workstations" -PassThru
$Status.text = "Creating Email Account $SF.$SL.student@$Domain"
$Wait
gam create user "$SF.$SL.student@$Domain" firstname "$SF" lastname "$SL" password "$6to12pwd" changepassword "on" org "Middle School Students (Restricted Mail)/Year $SY Students" organization department "Year 0$SY" primary
$Status.text = "Adding User $SF $SL to Year$SYStudents Email Group.."
$Wait
gam update group "Year$SYStudents" add "member" user "$SF.$SL.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientITname <$EmailrecipientIT@$Domain>" -Subject "Please prepare a laptop for $SF $SL" -Body "Hi IT Support, `n`nI've just created a computer and email account for $SF $SL who will be in year $SY.`n`nPlease prepare a laptop for them to use." -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Creating Email Account $SF.$SL.student@$Domain"
$Wait
gam create user "$SF.$SL.student@$Domain" firstname "$SF" lastname "$SL" password "$6to12pwd" changepassword "on" org "Senior School Students/Year $SY Students" organization department "Year $SY" primary
$Status.text = "Adding User $SF $SL to Year$SYStudents Email Group.."
$Wait
gam update group "Year$SYStudents" add "member" user "$SF.$SL.student@$Domain"
$Status.text = "Sending Email.."
...
if ($S1N -match “[.]”) {[System.Windows.Forms.MessageBox]::Show("$S1N contains a '.' please correct the format", 'Error', 'OK', 'Error')}
if ($S2N -match “[.]”) {[System.Windows.Forms.MessageBox]::Show("$S2N contains a '.' please correct the format", 'Error', 'OK', 'Error')}
if ($S3N -match “[.]”) {[System.Windows.Forms.MessageBox]::Show("$S3N contains a '.' please correct the format", 'Error', 'OK', 'Error')}
if ($S4N -match “[.]”) {[System.Windows.Forms.MessageBox]::Show("$S4N contains a '.' please correct the format", 'Error', 'OK', 'Error')}
if ($S5N -match “[.]”) {[System.Windows.Forms.MessageBox]::Show("$S5N contains a '.' please correct the format", 'Error', 'OK', 'Error')}
...
$Wait
New-ADUser -Name "$S1F $S1L"  -UserPrincipalName "$S1F.$S1L@$Domain" -SamAccountName "$S1F.$S1L" -GivenName $S1F -DisplayName "$S1F $S1L" -SurName "$S1L" -Description "Junior School" -EmailAddress "$S1F.$S1L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S1F.$S1L" -Office "$S1Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S1Y,OU=Junior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
...
$Wait
gam create user "$S1F.$S1L.student@$Domain" firstname "$S1F" lastname "$S1L" password "$simplepass" changepassword "on" org "Primary School (No Mail)/Year $S1Y Students" organization department "Year 0$S1Y" primary
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientJPname <$EmailrecipientJP@$Domain>" -Subject "A login has been created for $S1F $S1L" -Body "Hi $EmailrecipientJPname, `n`nI've just created a computer and email account for $S1F $S1L who will be in year $S1Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}

elseif ($S1Y -In 3..5) { 
$Status.text = "Creating AD Account for $S1F $S1L.."
$Wait
New-ADUser -Name "$S1F $S1L"  -UserPrincipalName "$S1F.$S1L@$Domain" -SamAccountName "$S1F.$S1L" -GivenName $S1F -DisplayName "$S1F $S1L" -SurName "$S1L" -Description "Primary School" -EmailAddress "$S1F.$S1L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S1F.$S1L" -Office "$S1Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S1Y,OU=Primary School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
$Status.text = "Adding User $S1F $S1L to Year $S1Y Students Group.."
$Wait
Add-ADGroupMember -Identity "Year $S1Y Students" -Members "$S1F.$S1L" -PassThru
$Status.text = "Adding User $S1F $S1L to gpOUStudentsYr$S1Y Group.."
...
$Status.text = "Creating Email Account $S1F.$S1L.student@$Domain"
$Wait
gam create user "$S1F.$S1L.student@$Domain" firstname "$S1F" lastname "$S1L" password "$6to12pwd" changepassword "on" org "Primary School (No Mail)/Year $S1Y Students" organization department "Year 0$S1Y" primary
$Status.text = "Adding User $S1F $S1L to Year$S1YStudents Email Group.."
$Wait
gam update group "Year$S1YStudents" add "member" user "$S1F.$S1L.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientPSname <$EmailrecipientPS@$Domain>" -Subject "A login has been created for $S1F $S1L" -Body "Hi $EmailrecipientPSname, `n`nI've just created a computer and email account for $S1F $S1L who will be in year $S1Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Set Allowed Workstations"
$Wait
Set-ADUser -Identity "$S1F.$S1L" -LogonWorkstations "$Workstations" -PassThru
$Status.text = "Creating Email Account $S1F.$S1L.student@$Domain"
$Wait
gam create user "$S1F.$S1L.student@$Domain" firstname "$S1F" lastname "$S1L" password "$6to12pwd" changepassword "on" org "Middle School Students (Restricted Mail)/Year $S1Y Students" organization department "Year 0$S1Y" primary
$Status.text = "Adding User $S1F $S1L to Year$S1YStudents Email Group.."
$Wait
gam update group "Year$S1YStudents" add "member" user "$S1F.$S1L.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientITname <$EmailrecipientIT@$Domain>" -Subject "Please prepare a laptop for $S1F $S1L" -Body "Hi IT Support, `n`nI've just created a computer and email account for $S1F $S1L who will be in year $S1Y.`n`nPlease prepare a laptop for them to use." -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Creating Email Account $S1F.$S1L.student@$Domain"
$Wait
gam create user "$S1F.$S1L.student@$Domain" firstname "$S1F" lastname "$S1L" password "$6to12pwd" changepassword "on" org "Senior School Students/Year $S1Y Students" organization department "Year $S1Y" primary
$Status.text = "Adding User $S1F $S1L to Year$S1YStudents Email Group.."
$Wait
gam update group "Year$S1YStudents" add "member" user "$S1F.$S1L.student@$Domain"
$Status.text = "Sending Email.."
...
$Wait
New-ADUser -Name "$S2F $S2L"  -UserPrincipalName "$S2F.$S2L@$Domain" -SamAccountName "$S2F.$S2L" -GivenName $S2F -DisplayName "$S2F $S2L" -SurName "$S2L" -Description "Junior School" -EmailAddress "$S2F.$S2L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S2F.$S2L" -Office "$S2Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S2Y,OU=Junior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
...
$Wait
gam create user "$S2F.$S2L.student@$Domain" firstname "$S2F" lastname "$S2L" password "$simplepass" changepassword "on" org "Primary School (No Mail)/Year $S2Y Students" organization department "Year 0$S2Y" primary
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientJPname <$EmailrecipientJP@$Domain>" -Subject "A login has been created for $S2F $S2L" -Body "Hi $EmailrecipientJPname, `n`nI've just created a computer and email account for $S2F $S2L who will be in year $S2Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}

elseif ($S2Y -In 3..5) {
$Status.text = "Creating AD Account for $S2F $S2L.."
$Wait
New-ADUser -Name "$S2F $S2L"  -UserPrincipalName "$S2F.$S2L@$Domain" -SamAccountName "$S2F.$S2L" -GivenName $S2F -DisplayName "$S2F $S2L" -SurName "$S2L" -Description "Primary School" -EmailAddress "$S2F.$S2L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S2F.$S2L" -Office "$S2Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S2Y,OU=Primary School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
$Status.text = "Adding User $S2F $S2L to Year $S2Y Students Group.."
$Wait
Add-ADGroupMember -Identity "Year $S2Y Students" -Members "$S2F.$S2L" -PassThru
$Status.text = "Adding User $S2F $S2L to gpOUStudentsYr$S2Y Group.."
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientPSname <$EmailrecipientPS@$Domain>" -Subject "A login has been created for $S2F $S2L" -Body "Hi $EmailrecipientPSname, `n`nI've just created a computer and email account for $S2F $S2L who will be in year $S2Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Set Allowed Workstations"
$Wait
Set-ADUser -Identity "$S2F.$S2L" -LogonWorkstations "$Workstations" -PassThru
$Status.text = "Creating Email Account $S2F.$S2L.student@$Domain"
$Wait
gam create user "$S2F.$S2L.student@$Domain" firstname "$S2F" lastname "$S2L" password "$6to12pwd" changepassword "on" org "Middle School Students (Restricted Mail)/Year $S2Y Students" organization department "Year 0$S2Y" primary
$Status.text = "Adding User $S2F $S2L to Year$S2YStudents Email Group.."
$Wait
gam update group "Year$S2YStudents" add "member" user "$S2F.$S2L.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientITname <$EmailrecipientIT@$Domain>" -Subject "Please prepare a laptop for $S2F $S2L" -Body "Hi IT Support, `n`nI've just created a computer and email account for $S2F $S2L who will be in year $S2Y.`n`nPlease prepare a laptop for them to use." -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Creating Email Account $S2F.$S2L.student@$Domain"
$Wait
gam create user "$S2F.$S2L.student@$Domain" firstname "$S2F" lastname "$S2L" password "$6to12pwd" changepassword "on" org "Senior School Students/Year $S2Y Students" organization department "Year $S2Y" primary
$Status.text = "Adding User $S2F $S2L to Year$S2YStudents Email Group.."
$Wait
gam update group "Year$S2YStudents" add "member" user "$S2F.$S2L.student@$Domain"
$Status.text = "Sending Email.."
...
$Wait
New-ADUser -Name "$S3F $S3L"  -UserPrincipalName "$S3F.$S3L@$Domain" -SamAccountName "$S3F.$S3L" -GivenName $S3F -DisplayName "$S3F $S3L" -SurName "$S3L" -Description "Junior School" -EmailAddress "$S3F.$S3L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S3F.$S3L" -Office "$S3Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S3Y,OU=Junior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
...
$Wait
gam create user "$S3F.$S3L.student@$Domain" firstname "$S3F" lastname "$S3L" password "$simplepass" changepassword "on" org "Primary School (No Mail)/Year $S3Y Students" organization department "Year 0$S3Y" primary
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientJPname <$EmailrecipientJP@$Domain>" -Subject "A login has been created for $S3F $S3L" -Body "Hi $EmailrecipientJPname, `n`nI've just created a computer and email account for $S3F $S3L who will be in year $S3Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}

elseif ($S3Y -In 3..5) {
$Status.text = "Creating AD Account for $S3F $S3L.."
$Wait
New-ADUser -Name "$S3F $S3L"  -UserPrincipalName "$S3F.$S3L@$Domain" -SamAccountName "$S3F.$S3L" -GivenName $S3F -DisplayName "$S3F $S3L" -SurName "$S3L" -Description "Primary School" -EmailAddress "$S3F.$S3L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S3F.$S3L" -Office "$S3Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S3Y,OU=Primary School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
$Status.text = "Adding User $S3F $S3L to Year $S3Y Students Group.."
$Wait
Add-ADGroupMember -Identity "Year $S3Y Students" -Members "$S3F.$S3L" -PassThru
$Status.text = "Adding User $S3F $S3L to gpOUStudentsYr$S3Y Group.."
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientPSname <$EmailrecipientPS@$Domain>" -Subject "A login has been created for $S3F $S3L" -Body "Hi $EmailrecipientPSname, `n`nI've just created a computer and email account for $S3F $S3L who will be in year $S3Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Set Allowed Workstations"
$Wait
Set-ADUser -Identity "$S3F.$S3L" -LogonWorkstations "$Workstations" -PassThru
$Status.text = "Creating Email Account $S3F.$S3L.student@$Domain"
$Wait
gam create user "$S3F.$S3L.student@$Domain" firstname "$S3F" lastname "$S3L" password "$6to12pwd" changepassword "on" org "Middle School Students (Restricted Mail)/Year $S3Y Students" organization department "Year 0$S3Y" primary
$Status.text = "Adding User $S3F $S3L to Year$S3YStudents Email Group.."
$Wait
gam update group "Year$S3YStudents" add "member" user "$S3F.$S3L.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientITname <$EmailrecipientIT@$Domain>" -Subject "Please prepare a laptop for $S3F $S3L" -Body "Hi IT Support, `n`nI've just created a computer and email account for $S3F $S3L who will be in year $S3Y.`n`nPlease prepare a laptop for them to use." -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Creating Email Account $S3F.$S3L.student@$Domain"
$Wait
gam create user "$S3F.$S3L.student@$Domain" firstname "$S3F" lastname "$S3L" password "$6to12pwd" changepassword "on" org "Senior School Students/Year $S3Y Students" organization department "Year $S3Y" primary
$Status.text = "Adding User $S3F $S3L to Year$S3YStudents Email Group.."
$Wait
gam update group "Year$S3YStudents" add "member" user "$S3F.$S3L.student@$Domain"
$Status.text = "Sending Email.."
...
$Wait
New-ADUser -Name "$S4F $S4L"  -UserPrincipalName "$S4F.$S4L@$Domain" -SamAccountName "$S4F.$S4L" -GivenName $S4F -DisplayName "$S4F $S4L" -SurName "$S4L" -Description "Junior School" -EmailAddress "$S4F.$S4L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S4F.$S4L" -Office "$S4Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S4Y,OU=Junior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
...
$Wait
gam create user "$S4F.$S4L.student@$Domain" firstname "$S4F" lastname "$S4L" password "$simplepass" changepassword "on" org "Primary School (No Mail)/Year $S4Y Students" organization department "Year 0$S4Y" primary
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientJPname <$EmailrecipientJP@$Domain>" -Subject "A login has been created for $S4F $S4L" -Body "Hi $EmailrecipientJPname, `n`nI've just created a computer and email account for $S4F $S4L who will be in year $S4Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}

elseif ($S4Y -In 3..5) {
$Status.text = "Creating AD Account for $S4F $S4L.."
$Wait
New-ADUser -Name "$S4F $S4L"  -UserPrincipalName "$S4F.$S4L@$Domain" -SamAccountName "$S4F.$S4L" -GivenName $S4F -DisplayName "$S4F $S4L" -SurName "$S4L" -Description "Primary School" -EmailAddress "$S4F.$S4L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S4F.$S4L" -Office "$S4Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S4Y,OU=Primary School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
$Status.text = "Adding User $S4F $S4L to Year $S4Y Students Group.."
$Wait
Add-ADGroupMember -Identity "Year $S4Y Students" -Members "$S4F.$S4L" -PassThru
$Status.text = "Adding User $S4F $S4L to gpOUStudentsYr$S4Y Group.."
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientPSname <$EmailrecipientPS@$Domain>" -Subject "A login has been created for $S4F $S4L" -Body "Hi $EmailrecipientPSname, `n`nI've just created a computer and email account for $S4F $S4L who will be in year $S4Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Set Allowed Workstations"
$Wait
Set-ADUser -Identity "$S4F.$S4L" -LogonWorkstations "$Workstations" -PassThru
$Status.text = "Creating Email Account $S4F.$S4L.student@$Domain"
$Wait
gam create user "$S4F.$S4L.student@$Domain" firstname "$S4F" lastname "$S4L" password "$6to12pwd" changepassword "on" org "Middle School Students (Restricted Mail)/Year $S4Y Students" organization department "Year 0$S4Y" primary
$Status.text = "Adding User $S4F $S4L to Year$S4YStudents Email Group.."
$Wait
gam update group "Year$S4YStudents" add "member" user "$S4F.$S4L.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientITname <$EmailrecipientIT@$Domain>" -Subject "Please prepare a laptop for $S4F $S4L" -Body "Hi IT Support, `n`nI've just created a computer and email account for $S4F $S4L who will be in year $S4Y.`n`nPlease prepare a laptop for them to use." -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Creating Email Account $S4F.$S4L.student@$Domain"
$Wait
gam create user "$S4F.$S4L.student@$Domain" firstname "$S4F" lastname "$S4L" password "$6to12pwd" changepassword "on" org "Senior School Students/Year $S4Y Students" organization department "Year $S4Y" primary
$Status.text = "Adding User $S4F $S4L to Year$S4YStudents Email Group.."
$Wait
gam update group "Year$S4YStudents" add "member" user "$S4F.$S4L.student@$Domain"
$Status.text = "Sending Email.."
...
$Wait
New-ADUser -Name "$S5F $S5L"  -UserPrincipalName "$S5F.$S5L@$Domain" -SamAccountName "$S5F.$S5L" -GivenName $S5F -DisplayName "$S5F $S5L" -SurName "$S5L" -Description "Junior School" -EmailAddress "$S5F.$S5L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S5F.$S5L" -Office "$S5Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S5Y,OU=Junior School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
...
$Wait
gam create user "$S5F.$S5L.student@$Domain" firstname "$S5F" lastname "$S5L" password "$simplepass" changepassword "on" org "Primary School (No Mail)/Year $S5Y Students" organization department "Year 0$S5Y" primary
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientJPname <$EmailrecipientJP@$Domain>" -Subject "A login has been created for $S5F $S5L" -Body "Hi $EmailrecipientJPname, `n`nI've just created a computer and email account for $S5F $S5L who will be in year $S5Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}

elseif ($S5Y -In 3..5) {
$Status.text = "Creating AD Account for $S5F $S5L.."
$Wait
New-ADUser -Name "$S5F $S5L"  -UserPrincipalName "$S5F.$S5L@$Domain" -SamAccountName "$S5F.$S5L" -GivenName $S5F -DisplayName "$S5F $S5L" -SurName "$S5L" -Description "Primary School" -EmailAddress "$S5F.$S5L.student@$Domain" -Department "Students" -HomeDrive H: -HomeDirectory "\\$Domain\Student\StudentHome\$S5F.$S5L" -Office "$S5Y" -ProfilePath "\\$Domain\Student\StudentProfiles\Mandatory" -Path "OU=Year $S5Y,OU=Primary School,OU=Students,DC=hopecc,DC=sa,DC=edu,DC=au" -AccountPassword (ConvertTo-SecureString "$simplepass" -AsPlainText -force) -Enabled $True -CannotChangePassword $True -PasswordNeverExpires $True -PassThru
$Status.text = "Adding User $S5F $S5L to Year $S5Y Students Group.."
$Wait
Add-ADGroupMember -Identity "Year $S5Y Students" -Members "$S5F.$S5L" -PassThru
$Status.text = "Adding User $S5F $S5L to gpOUStudentsYr$S5Y Group.."
...
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientPSname <$EmailrecipientPS@$Domain>" -Subject "A login has been created for $S5F $S5L" -Body "Hi $EmailrecipientPSname, `n`nI've just created a computer and email account for $S5F $S5L who will be in year $S5Y.`n`nTheir password is $simplepass" -SmtpServer "aspmx.l.google.com"
}
elseif ($S5Y -In 6..9) {
$Status.text = "Creating AD Account for $S5F $S5L.."
$Wait
...
$Status.text = "Set Allowed Workstations"
$Wait
Set-ADUser -Identity "$S5F.$S5L" -LogonWorkstations "$Workstations" -PassThru
$Status.text = "Creating Email Account $S5F.$S5L.student@$Domain"
$Wait
gam create user "$S5F.$S5L.student@$Domain" firstname "$S5F" lastname "$S5L" password "$6to12pwd" changepassword "on" org "Middle School Students (Restricted Mail)/Year $S5Y Students" organization department "Year 0$S5Y" primary
$Status.text = "Adding User $S5F $S5L to Year$S5YStudents Email Group.."
$Wait
gam update group "Year$S5YStudents" add "member" user "$S5F.$S5L.student@$Domain"
$Status.text = "Sending Email.."
$Wait
Send-MailMessage -From "$emailsendername <$emailsender@$Domain>" -To "$EmailrecipientITname <$EmailrecipientIT@$Domain>" -Subject "Please prepare a laptop for $S5F $S5L" -Body "Hi IT Support, `n`nI've just created a computer and email account for $S5F $S5L who will be in year $S5Y.`n`nPlease prepare a laptop for them to use." -SmtpServer "aspmx.l.google.com"
}
...
$Status.text = "Creating Email Account $S5F.$S5L.student@$Domain"
$Wait
gam create user "$S5F.$S5L.student@$Domain" firstname "$S5F" lastname "$S5L" password "$6to12pwd" changepassword "on" org "Senior School Students/Year $S5Y Students" organization department "Year $S5Y" primary
$Status.text = "Adding User $S5F $S5L to Year$S5YStudents Email Group.."
$Wait
gam update group "Year$S5YStudents" add "member" user "$S5F.$S5L.student@$Domain"
$Status.text = "Sending Email.."
