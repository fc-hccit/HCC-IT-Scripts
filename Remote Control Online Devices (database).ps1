...
if((Test-Connection $ListComputer -Count 1 -Quiet ) -eq $true) {Start "C:\Program Files (x86)\Microsoft Configuration Manager\AdminConsole\bin\i386\CmRcViewer.exe" $ListComputer
                                                                Start "C:\Program Files (x86)\Microsoft Endpoint Manager\AdminConsole\bin\i386\CmRcViewer.exe" $ListComputer}
...
"$User - $computer" | out-file ".\$selection Database.txt" -Force -append
...
$getunique = Get-content ".\$Selection Database.txt" | sort | get-unique | out-file ".\updatedDB.txt"
Remove-Item ".\$Selection Database.txt"
Rename-Item ".\updatedDB.txt" -NewName ".\$Selection Database.txt"

$Button1.text = "Update DB"
...
$Selection = $ComboBox1.SelectedItem.ToString()
$ListBox1.Items.Clear()
Get-Content ".\$selection Database.txt" | ForEach-Object {[void] $listbox1.Items.Add("$_")}
$Button1.text = "Update DB"
}
...
$Form.text                       = "Remote Control Online Devices"
$Form.TopMost                    = $true
...
$Selection = $ComboBox1.SelectedItem.ToString()
if($S1YR.SelectedIndex -ne "-1") {$Button1.Enabled = $true}
if(test-path ".\$selection Database.txt"){ $Button1.text = "Load DB"}
else{$Button1.text = "Create DB"}
})
...
$Button1.Add_Click({
$button = $Button1.Text
if ($Button1.text -eq "Update DB"){write-host "running update";Create}
if ($Button1.text -eq "Load DB"){write-host "running load";Load}
if ($Button1.text -eq "Create DB"){write-host "running create";Create}
